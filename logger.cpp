...
ostream &operator<<(ostream &stream, const string msg) { return stream << msg; }
...
string Log::GetTime() {
    ...
    sprintf(str, "%d-%02d-%02d %02d:%02d:%02d", time_tm->tm_year + 1900,
            time_tm->tm_mon + 1, time_tm->tm_mday, time_tm->tm_hour,
            time_tm->tm_min, time_tm->tm_sec);
    for (int i = 0; i < 19; i++)
        strTime.push_back(str[i]);
    return strTime;
}
...
void ConsoleLogger::Output(const char *tm, const char *nLevel,
                           const char *msg) {
    printf("[%s][%s]%s\n", tm, nLevel, msg);
}
